...
void Itinerary::print(Fleet& fleet, std::ostream& out = std::cout) {
	out << fleet.name(legs[legs.size() - 1].id) << "\t" << origin->name << "\t" << legs[legs.size() - 1].departure_time << "\t" << destinations[destinations.size() - 1] << "\t" << legs[legs.size() - 1].arrival_time << std::endl;
	...
	out << "Route Stats->" << " Total Time In Space: " << fTimeInSpace << " Overall Travel Time: " << fTotalTravelTime << std::endl;
}
...
void Planet::dump(Galaxy* galaxy) {
	...
	if (predecessor == nullptr) {
		...
		std::cerr << std::setw(5) << "Best Leg: Predecessor:" << predecessor->name << "Ship: " << galaxy->fleet.name(best_leg.id) <<" Departure: " << best_leg.departure_time << " Arrival: " << arrival_time << std::endl;
	}
...
void Planet::relax_neighbors(PriorityQueue<Planet, int(*)(Planet*, Planet*)>& queue) {
	int layoverEnd = best_leg.arrival_time + 4;
	for (auto edge : edges) {
...
void Galaxy::search() {
	...
	for (auto planet : planets) {
		...
		if(destinationItin->fTotalTravelTime > fFurthestPlanetItin->fTotalTravelTime) {
